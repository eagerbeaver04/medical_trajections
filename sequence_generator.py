...
from patient_statuses import PatientCondition, PATIENT_CONDITION_LEN, get_padding_conditions
class MedicalSequence:
    def __init__(self, status_sequence:  Optional[list[PatientCondition]] = None, event_sequence: Optional[list[Event]] = None):
    ...
    def append(self, status: PatientCondition, event: Event):
    ...
    def pad(self, length: int):
        ...
        padded_status_sequence = self._status_sequence + [get_padding_conditions()] * (length - len(self._status_sequence))
